...
<template>
  <div class="flex flex-col bg-grid">
    ...
    <div v-if="highlightedCode && formattedCode" v-show="rendererReady" class="relative w-full p-3">
      ...
      <pre class="p-4 min-h-40 max-h-72 text-sm overflow-y-auto rounded-[calc(var(--ui-radius)*1.5)] border border-(--ui-border) bg-neutral-50 dark:bg-neutral-800" v-html="highlightedCode" />
      <UButton
        color="neutral"
        variant="link"
